...
temp_daily <- temp_sum %>%
  ...
  filter(!is.na(wytype), (month >1 & month < 6), wy > 2008 & wy< 2020) %>%
...
plot_temp <- ggplot2::ggplot()+
  ...
  ggplot2::geom_point(data = firsttemp, aes(x = date2, y = 12, color = station), size = 2, shape = 5) + facet_wrap(~fWY, strip.position = "left", nrow = 11)+
